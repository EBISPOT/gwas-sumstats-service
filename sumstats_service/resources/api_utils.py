...
def validate_files_from_payload(callback_id, content):
    if config.VALIDATE_WITH_SSH == 'true':
        ...
        bsub_com = 'SINGULARITY_CACHEDIR={cache} singularity exec --home {home} --bind {sp} docker://{image}:{tag} validate-payload -cid {cid} -out {outfile} -storepath {sp} -payload \'{content}\''.format(
                image=config.SINGULARITY_IMAGE, tag=config.SINGULARITY_TAG, cid=callback_id, outfile=outfile, sp=config.STORAGE_PATH, content=content, cache=config.CACHE_PATH, home=config.USERHOME)
